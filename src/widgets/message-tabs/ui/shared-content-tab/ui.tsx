...
export const SharedContentTab: React.FC<SharedContentTabProps> = ({
    ...
    useEffect(() => {
        if (isClient) return;
        setSharedItems(mapCoachSharedContent(coachSharedContent?.data?.shared_content));
    ...
    const handleOpenDocument = (contentId: string) => {
        ...
        const folderId = findFolderForContent(foldersResponse?.folders || [], contentId);
        ...
        navigate(`/content-manager/library/folder/${folderId}/document/${contentId}`);
    };
    ...
    return (
        ...
        >
            {!sharedItems.length ? (
                ...
                <div className={cn("flex flex-col gap-4", !isMobileOrTablet && "lg:p-6")}>

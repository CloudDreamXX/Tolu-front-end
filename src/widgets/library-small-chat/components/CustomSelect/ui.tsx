    ...
    return (
        <div className={`relative ${className}`} ref={dropdownRef}>
            ...
            >
                ...
                {isOpen ? <ChevronUp className="w-[20px] h-[20px]" /> : <ChevronDown className="w-[20px] h-[20px]" />}
            </button>
            ...
            {isOpen && (
                <ul className="absolute z-10 mt-[4px] w-full min-w-[113px] bg-white border border-[#DBDEE1] rounded-[12px] shadow-sm max-h-[220px] overflow-y-auto text-[16px]">
                    {options.map((option, index) => (
                        <li
                            key={option}
                            className={`px-[17px] py-[8px] cursor-pointer hover:text-[#1C63DB] ${value === option ? "text-[#008FF6] font-semibold" : ""

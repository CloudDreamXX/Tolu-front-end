...
export const SearchableSelect: React.FC<SearchableSelectProps> = ({
    ...
    return (
        <div className="w-[100%] relative" ref={containerRef}>
            <label className={`peer-focus:text-[#1D1D1F] ${labelStyle} font-[Nunito] text-[16px] font-medium text-[#1D1D1F] mb-2 block`}>{label}</label>
            <input
                type="text"
                value={inputValue}
            ...
            <div className={`pointer-events-none absolute right-4 top-[48px] transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`}>
                <svg

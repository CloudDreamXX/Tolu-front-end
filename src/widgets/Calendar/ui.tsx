import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, X, Clock, Trash2 } from 'lucide-react';
...
export const CalendarPopup = () => {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    dayGrid.push(
      ...
      >
        <div className="text-right text-xs text-[#71717A] pr-1">{day.getDate()}</div>
        {dayEvents.map((event, idx) => {
          const index = events.findIndex(
            e => e.date === event.date && e.time === event.time && e.text === event.text
          );
          return (
            <div
              key={idx}
            ...
            >
              ...
              <span className="text-[12px] text-[#1D1D1F] leading-[16px] truncate">{event.text}</span>
            </div>
          );
        })}
      </div>
  ...
  return (
    <div className="absolute right-6 top-[164px] rounded-2xl bg-white shadow-md flex flex-col py-8 px-6 gap-6 w-[924px] h-[740px]">
      <h2 className="text-[24px] leading-[32px] font-semibold text-[#1D1D1F] font-[Nunito]">Network Support</h2>

      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2 w-full max-w-[256px]">
          <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="h-[44px] w-[44px] bg-[#DDEBF6] flex items-center justify-center rounded-[8px]">
          ...
          <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="h-[44px] w-[44px] bg-[#DDEBF6] flex items-center justify-center rounded-[8px]">
            <ChevronRight size={24} color="#1C63DB" />
          </button>
        </div>
        <button onClick={() => setCurrentMonth(new Date())} className="h-[44px] px-6 rounded-full bg-[#DDEBF6] text-[#1C63DB] font-[Nunito] text-[16px] font-semibold">
          Today
        </button>
      </div>
      ...
      {showModal && (
        <div className="absolute top-1/2 left-1/2 w-[360px] -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-xl z-50 p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input value={newEventName} onChange={e => setNewEventName(e.target.value)} placeholder="Enter event name" className="border border-[#E4E4E7] rounded px-3 py-2 text-sm" />
          </div>
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center border border-[#E4E4E7] rounded px-3 py-2">
              ...
              <input value={newEventTime} onChange={e => setNewEventTime(e.target.value)} className="w-full text-sm" placeholder="09:00 AM" />
            </div>
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm text-[#1D1D1F]">Notes <span className="text-[#A1A1AA]">(Optional)</span></label>
            <input value={newEventNote} onChange={e => setNewEventNote(e.target.value)} placeholder="Leave short note" className="border border-[#E4E4E7] rounded px-3 py-2 text-sm" />
          </div>
          <div className="flex justify-between mt-4 items-center">
            {editIndex !== null ? (
              <button onClick={handleDeleteEvent} className="flex items-center gap-2 text-red-500 font-semibold text-sm">
                <Trash2 size={16} /> Delete event
              </button>
            ) : <div />}
            <button onClick={handleAddEvent} className="rounded-full bg-[#1C63DB] px-4 py-2 text-white font-semibold text-sm">
      ...
      <Dialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <DialogContent className='w-[742px]'>
          <DialogHeader className='flex flex-col gap-6 p-6 items-start'>
            ...
            <DialogDescription>Deleting it will permanently remove event from your calendar. This action cannot be undone, and you won't be able to recover the event later.</DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex justify-between items-center w-full">
            <button onClick={() => setShowDeleteConfirm(false)} className="flex items-center rounded-full justify-center w-32 h-[44px] text-sm p-4 bg-[#DDEBF6] text-[#1C63DB]">Cancel</button>
            <button onClick={confirmDeleteEvent} className="w-32 h-[44px] text-sm p-4 rounded-full bg-[#FF1F0F] text-white flex items-center justify-center">Delete</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>

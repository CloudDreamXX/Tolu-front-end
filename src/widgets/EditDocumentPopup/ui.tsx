...
export const EditDocumentPopup: React.FC<Props> = ({ onEdit, onMove, onDublicate, onMarkAs, onArchive, onDelete, position, type }) => {
    return (
        <div
            ...
            style={{ top: type === "subfolder" ? position.top - 200 + window.scrollY : position.top - 280 + window.scrollY, left: position.left - 210 }}
        >
            <MenuItem icon={<Edit />} label="Edit" onClick={onEdit} />
            {type !== "subfolder" && <MenuItem icon={<ArrowRight />} label="Move" onClick={onMove} />}
            <MenuItem icon={<Dublicate />} label="Duplicate" onClick={onDublicate} />
            {type !== "subfolder" && <MenuItem
                icon={<Eye width={24} height={24} />}
                label="Mark as"
                onClick={onMarkAs}
            />}
            <div className="h-[1px] w-full bg-[#E3E3E3]" />
            <MenuItem icon={<Archive size={24} />} label="Archive" onClick={onArchive} />
            <MenuItem icon={<Trash />} label="Delete" className="text-[#FF1F0F]" onClick={onDelete} />
        </div>
    );
};

...
export const PopoverClient: React.FC<IPopoverClientProps> = ({
  ...
  const filteredClients = useMemo(() => {
    return clients.filter((client) =>
      client.name && client.name.toLowerCase().includes(search.toLowerCase()) || client.first_name.toLowerCase().includes(search.toLowerCase()) || client.last_name.toLowerCase().includes(search.toLowerCase())
    );
  }, [search, clients]);
  ...
  return (
    <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
      ...
      <PopoverContent className="w-[358px] md:w-[419px] p-6 flex flex-col z-50">
        ...
        />
        <ScrollArea
          className={`${filteredClients.length > 2 ? "h-[139px]" : "h-[79px]"} w-full`}
        >

...
const StepperModal = ({ handleAction, closeModal }) => {
  ...
  const renderDropdowns = (modalType) => {
    const data =
      modalType === 1
        ? symptomCategories.slice(0, 7)
        : modalType === 2
          ? symptomCategories.slice(7, 14)
          : symptomCategories.slice(14, 15);

    return data.map((category, index) => {
      const selectedValue =
        modalType === 1
          ? selectedModal1[index]
          : modalType === 2
            ? selectedModal2[index]
            : selectedModal3[index];
      return (
      ...
      return (
        <>
          <section className="flex flex-col items-center">
            <h3 className="text-2xl text-[#252525B2] font-bold mb-8">
    ...
    if (modalIndex === 1) {
      return (
        <>
          <section className="flex flex-col items-center">
            <h3 className="text-2xl text-[#252525B2] font-bold mb-8">
              Select your diagnosed conditions
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-4">
              {renderDropdowns(2)}
            </div>

...
const MainChat = () => {
  ...
  const filteredData =
    data?.history?.filter((item) => item.chat_id === selectedChatId) || [];
      ...
      setChats((prevChats) => {
        ...
        updatedChats[updatedChats.length - 1] = {
          ...
          result_id:
            ...
            updatedChats[updatedChats.length - 1]?.result_id,
          chat_id:
            response.data.chat_id ||
            updatedChats[updatedChats.length - 1].chat_id,
        };
        return updatedChats;
      });
  ...
  const [updateChatTitle, { isLoading: titleLoading }] =
    useUpdateChatTitleMutation();
    ...
    <div className="flex gap-2 grow ">
      <div className="flex items-end w-full">
        <div className="text-center flex flex-col h-[90vh] gap-7 items-center w-full">
          <div
            className={`my-2 p-2 w-full flex-1 overflow-y-auto ${chats.length ? 'h-[400px]' : 'h-0'}`}
          >
            {chats.length > 0 && (
              <>
                <section className="flex gap-8 ">
                  ...
                  <Button
                    ...
                    text="Deleted Search"
                    className="border text-xs font-normal !h-9"
                  >
